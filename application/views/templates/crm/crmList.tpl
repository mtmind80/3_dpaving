...
<div >
    ...
    <div class="panel-heading">
            <form method="post" action="{$SITE_URL}crm/showList/" id="searchform" name="searchform">
                ...
                Qualified: <select  name="cntQualified" id="cntQualified">
                    ...
                    {IF $data['filter']}
                    <a href="{$SITE_URL}crm/showList/1" class="btn btn-sm btn-pa-purple btn-labeled"><span class="btn-label icon fa fa-list"></span> Show All</a>
                    {/IF}
                    {IF $USER_ROLE == 1}
                    <a href="{$SITE_URL}crm/exportList/" class="btn btn-sm btn-light-green "><span class="btn-label icon fa fa-save"></span> Export</a>
                    {/IF}
                </p>
            </form>
        {IF $data['filter']}
                ...
                {foreach $data.rows as $d}
                    <tr class="{cycle values="$odd,$even"}">
                        <td><a href="{$SITE_URL}crm/editContact/{$d.cntId}" title="Click to view contact details">{IF $d.cntSalutation}{$d.cntSalutation} {/IF}{$d.cntFirstName} {$d.cntLastName}</a>
                                ...
                                {IF $d.project_count}
                                <br /><a href='{$SITE_URL}workorders/showPOList/{$d.cntId}'<span class="badge badge-primary">Projects:{$d.project_count}</span></a>
                                {/IF}
                                &nbsp;
                            </td>
                    ...
                    {IF $d.cntPrimaryAddress2}<br/>{$d.cntPrimaryAddress2}{/IF}
                        ...
                        <br/><a href="mailto:{$d.cntPrimaryEmail}" title="Send Email">{$d.cntPrimaryEmail}<a/>                    </td>
                        {/if}
                    <!--
                    <td class="text-left">{$d.cntUpdatedDate}</td>
                    -->
                    <td valign='top'>
                        {IF $USER_ROLE <= 3 OR $USER_ROLE eq 6}{* not allowed to add new contact if 4 or 5*}
					...
					{/IF}
                                    ...
                                    <!-- <br /><a  href="{$SITE_URL}comingsoon/{$d.cntId}">Upload Image</a>
        -->
                                </td></table>

<!--                        <a href="Javascript:ShowTools('{$d.cntFirstName} {$d.cntLastName}','{$d.cntId}', 'showTools');" title="Show Tools"><img src="{$SITE_URL}/images/tools2.gif" border="0" alt="Tools" width="26" height="26" VALIGN='top'></a>
-->
    ...
    <div class="DT-label">
        <div aria-relevant="all" aria-live="polite" role="alert" id="jq-datatables-example_info" class="dataTables_info">
            Records found22: {$data['total_records']} &nbsp; Page {$data['page_num']} of {$data['total_pages']} pages &nbsp; &nbsp;
            SHOW: &nbsp;
    ...
    <div class="DT-pagination">
        <div id="jq-datatables-example_paginate" class="dataTables_paginate paging_simple_numbers">
            <ul class="pagination">
                <li id="jq-datatables-example_previous" tabindex="0" aria-controls="jq-datatables class="paginate_button previous {IF $data['page_num'] <= 1}disabled {/IF}"><a href="Javascript:GoToPage({$data['page_num'] - 1});"><< Previous</a></li>
                <!--
                                    ...
                                    -->
                <li id="jq-datatables-example_next" tabindex="0" aria-controls="jq-datatables" class="paginate_button next {IF $data['page_num'] >= $data['total_pages']} disabled {/IF} "><a href="Javascript:GoToPage({$data['page_num'] + 1});">Next >></a></li>
            </ul>
...
<form method="post" action="{$SITE_URL}crm/showList/" name="paginate" id="paginate">
